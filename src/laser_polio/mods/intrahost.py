...
def init(model):
    ...
    try:
        ...
        shared_lib_path = resource_filename("laser_polio", "mods/libintrahost.so")
...
@nb.njit((nb.uint32, nb.uint8[:], nb.uint8[:], nb.uint16[:], nb.uint8[:], nb.float32, nb.float32, nb.uint32), parallel=True)
...
def step1(model, tick):
    ...
    # if use_nb:
    _exposure_update(
        nb.uint32(model.population.count),
...
delta = 1  # left over from agggressive experimentation
...
def step2(model, tick):
    ...
    if True:  # use_nb:
        ...
        lib.infection_update_contiguous(
