...
def make_cbr_dict():
    with Path("data00.json").open("r") as fid01:
        dict_adm00 = json.load(fid01)

    with Path("data01.json").open("r") as fid01:
        dict_adm01 = json.load(fid01)

    with Path("NGA_NAMES_LEV02.csv").open("r") as fid01:
    ...
    with Path("cbr_NGA.json").open("w") as fid01:
