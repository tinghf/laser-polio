...
def test_births_generated():
    ...
    assert initial_population + sim.results.births.sum() - sim.results.deaths.sum() == np.sum(sim.people.disease_state > -1), 'Population did not increase by the correct amount'
