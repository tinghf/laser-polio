...
from laser_polio.demographics import cumulative_deaths
from laser_polio.mods import age_init
from laser_polio.mods import immunity
from laser_polio.numpynumba.population import ExtendedLF as Population
from laser_polio.numpynumba.population import check_hdf5_attributes
from laser_polio.utils import viz_pop
...
class Model:
    def __init__(self, params):
        ...
        self.manifest_path = Path(self.params.input_dir) / "manifest.py"
        self.manifest = None
        self._load_manifest()
    ...
    def save(self, filename):
        ...
        if age_init.age_data_manager.get_data() is None:
            raise ValueError("age_distribution uninitialized while saving")
    ...
    def _init_from_data(self):
        ...
        self.nodes.initial_infections = np.uint32(np.round(np.random.poisson(self.params.prevalence * self.initial_populations)))
    ...
    def _check_for_cached(self):
        ...
        Path(hdf5_directory).mkdir(parents=True, exist_ok=True)

        for filename in os.listdir(hdf5_directory):
            if filename.endswith(".h5"):
                hdf5_filepath = Path(hdf5_directory) / filename
                if age_init.age_data_manager.get_data() is None:
