...
def check_hdf5_attributes(hdf5_filename, initial_populations, age_distribution, cumulative_deaths, eula_age=None):
    ...
    with h5py.File(hdf5_filename, "r") as hdf:
        try:
            ...
            if (
                np.array_equal(initial_populations, file_initial_populations)
                and np.array_equal(age_distribution, file_age_distribution)
                and np.array_equal(cumulative_deaths, file_cumulative_deaths)
                and eula_age == file_eula_age
            ):
                return True
            else:
...
class ExtendedLF(LaserFrame):
    ...
    def save(
        self, filename: str, tail_number=0, initial_populations=None, age_distribution=None, cumulative_deaths=None, eula_age=None
    ) -> None:
    ...
    def eliminate_eulas(self, split_index: int):
        ...
        for key, value in self.__dict__.items():
            if (
                isinstance(value, np.ndarray) and value.size >= self._count
            ):  # for some reason the value to check against here has been really annoying
                try:
                    self.__dict__[key] = value[split_index:]
                    print(f"Cropping array {key}.")

                except Exception:
                    raise ValueError(f"Exception resizing {key} vector.") from None
