...
def init(model):
    try:
        ...
        shared_lib_path = resource_filename("laser_polio", "mods/libages.so")
        ...
        print("Failed to load libages.so.")
...
def step(model, tick):
    if lib:
        lib.update_ages_contiguous_shards(ctypes.c_int64(model.population.count), model.population.age, delta, tick)
    else:
        # TBD: Need to implement age update in numba
        pass
