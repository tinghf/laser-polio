...
ri.loc[:, 'immunity_ri_nOPV2'] = ((ri['dpt1'] - ri['dpt2']) * perdose +
                                  (ri['dpt2'] - ri['dpt3']) * (1 - (1 - perdose)**2) +
                                  ri['dpt3'] * (1 - (1 - perdose)**3))
...
if n_na > 0:
    print(f'There are {n_na} missing values in the immunity_ri_nOPV2 column. Filling in missing values with the mean from higher admin levels.')
...
assert ri['immunity_ri_nOPV2'].isna().sum() == 0, "There are still missing values in the immunity_ri_nOPV2 column after taking the adm1 or adm0 mean."
