...
def count_SEIRP(node_id, disease_state, paralyzed, n_nodes):
    ...
    I = np.zeros(n_nodes, dtype=np.int64)  # noqa: E741
...
class DiseaseState_ABM:
    def __init__(self, sim):
        ...
        sim.people.exposure_timer[: np.sum(self.pars.n_ppl)] = self.pars.dur_exp(
        ...
        if len(pars.init_immun) == 1:
            ...
            for node in self.nodes:
                ...
                for age_key, (age_min, age_max) in age_bins.items():
                    ...
                    eligible = (sim.people.date_of_birth[alive_in_node] * -1 >= age_min) & (
                        sim.people.date_of_birth[alive_in_node] * -1 < age_max
                    )
...
def fast_vaccination(
    node_id, disease_state, date_of_birth, ri_timer, sim_t, vx_prob_ri, results_ri_vaccinated, results_ri_protected, rand_vals
):
    """
...
class RI_ABM:
    def __init__(self, sim):
        ...
        sim.results.add_array_property(
            "ri_vaccinated", shape=(sim.pars.timesteps, len(sim.nodes)), dtype=np.int32
        )  # Track number of people vaccinated by RI
        sim.results.add_array_property(
            "ri_protected", shape=(sim.pars.timesteps, len(sim.nodes)), dtype=np.int32
