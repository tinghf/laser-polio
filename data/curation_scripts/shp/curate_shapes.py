...
Path(extract_dir).mkdirs(parents=True, exist_ok=True)

# Extract the GeoJSON files from the zip archive
extracted_geojson_files = [
    Path(extract_dir) / f for f in os.listdir(extract_dir) if f.endswith(".geojson")
...
if not extracted_geojson_files:
    with zipfile.ZipFile(zip_path, "r") as zip_ref:
        ...
        for geojson_file in geojson_files_in_zip:
            ...
            file_name = Path(geojson_file).name
            target_path = Path(extract_dir) / file_name
            with zip_ref.open(geojson_file) as source, Path(target_path).open("wb") as target:
                target.write(source.read())

# List all GeoJSON files in the extracted directory
extracted_geojson_files = [Path(extract_dir) / f for f in os.listdir(extract_dir) if f.endswith(".geojson")]
...
for shapefile, columns_to_clean, adm_level in shapefiles:
    ...
    gdf["ENDDATE"] = gdf["ENDDATE"].apply(lambda x: acceptable_date_str if "9999" in x else x)  # noqa: B023
    # Convert ENDDATE to datetime.date
    gdf["ENDDATEv2"] = pd.to_datetime(gdf["ENDDATE"], errors="coerce").dt.date
    today = datetime.datetime.now(tz=datetime.timezone(-datetime.timedelta(hours=8))).date()  # Pacific timezone
    ...
    columns_to_keep = [
        *columns_to_clean,  # all these columns and ...
        "ISO_2_CODE",
        "ISO_3_CODE",
        "dot_name",
        "GUID",
