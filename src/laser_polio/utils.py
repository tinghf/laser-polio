...
def load_csv_maybe_header(file_path):
    ...
    with Path(file_path).open() as f:
        ...
        has_header = any(
            item.isalpha()  # Check if item contains letters
            or "-" in item  # Check if item has minus/hyphen
            or float(item) < 0
            or float(item) > 1
...
def viz_pop(model, url="https://packages.idmod.org:443/artifactory/idm-data/LASER/ssa_shapes.zip", extract_to_dir="ssa_shapes"):
    ...
    scatter = ax.scatter(nn_longitudes, nn_latitudes, s=nn_sizes, c=nn_populations, norm=LogNorm(), cmap="inferno")
...
def combine_pdfs(output_filename=pdf_filename):
    ...
    pdf_files.remove(Path(output_filename))
    for pdf in pdf_files:
        print(f"Found and appending {pdf}.")
        merger.append(str(pdf))

    # Write out the combined PDF
    with Path(output_filename).open("wb") as fout:
        merger.write(fout)
