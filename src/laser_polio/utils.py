...
def date(start_date_str):
    if isinstance(start_date_str, pd.Series):
        return start_date_str.apply(lambda x: dt.datetime.strptime(x, "%Y-%m-%d").astimezone(dt.timezone.utc).date())
    elif isinstance(start_date_str, dt.date):
        return start_date_str
    else:
        return dt.datetime.strptime(start_date_str, "%Y-%m-%d").astimezone(dt.timezone.utc).date()
...
def daterange(start_date, days):
    ...
    if isinstance(start_date, str):
        start_date = dt.datetime.strptime(start_date, "%Y-%m-%d").astimezone(dt.timezone.utc).date()
...
def get_distance_matrix(distance_matrix_path, name_filter):
    ...
    file_extension = Path(distance_matrix_path).suffix
