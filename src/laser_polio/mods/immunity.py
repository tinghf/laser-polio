...
def init(model):
    initialize_susceptibility(
        model.population.count, model.population.dob, model.population.susceptibility, model.population.susceptibility_timer
    )
    print(f"{np.count_nonzero(model.population.susceptibility == 1) / model.population.count}% initially susceptible.")
    try:
        # Load the shared library
        shared_lib_path = files("laser_polio.mods").joinpath("libmi.so")
        ...
        print(f"Failed to load libmi.so. Will use numba. Error: {e}")
...
def step(model, tick):
    global lib, use_nb
    if use_nb:
        _update_susceptibility_based_on_sus_timer_nb(
            model.population.count, model.population.susceptibility_timer, model.population.susceptibility, tick, delta
        )
    else:
        lib.update_susceptibility_timer_strided_shards(
            model.population.count, model.population.susceptibility_timer, model.population.susceptibility, delta, tick
        )
    return
